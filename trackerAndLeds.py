...
# pitch radians range -0.6720 (up) to 0.5139 (down)
headPitch = 0
# yaw radians range -2.0875 (right) to 2.0875 (left)
headYaw = 0
...
try:
    while(1):
        ...
        ### Make sure the bot doesnt do anything when he shouldnt
            ...
            if((area > 40000)&(headYaw == 0)):
                ...
                if(headPitch == 0):
                    ...
                    if((y >= yMin) & (y < yLowUp)):
                        ...
                        if((x >= xLowUp) & (x <= xHighUp)): # FORWARD FAST
                            motionProxy.setWalkTargetVelocity(1, 0, 0, 1.0)
                            print "Forward Fast"

                        elif((x >= xMin) & (x < xLowUp)): # LEFT SLOW
                            motionProxy.setWalkTargetVelocity(0, 0, 0.3, 0.1)
                            print "leftSlow"

                        elif((x > xHighUp) & (x <= xMax)): # RIGHT SLOW
                            motionProxy.setWalkTargetVelocity(0, 0, -0.3, 0.1)
                            print "rightSlow"

                    #### If the ball is at the middle of the screen
                    elif((y >= yLowUp) & (y <= yHighUp)):

                        if((x >= xLowUp) & (x <= xHighUp)): # FORWARD SLOW
                            motionProxy.setWalkTargetVelocity(1, 0, 0, 0.5)
                            print "Forward slow"

                        elif((x >= xMin )& (x < xLowUp)): # LEFT MEDIUM
                            motionProxy.setWalkTargetVelocity(0, 0, 0.3, 0.2)
                            print "leftMedium"

                        elif((x > xHighUp) & (x <= xMax)): # RIGHT MEDIUM
                            motionProxy.setWalkTargetVelocity(0, 0, -0.3, 0.2)
                            print "rightMedium"

                    #### If the ball is at the bottom of the screen
                    elif((y > yHighUp) & (y <= yMax)):

                        if((x >= xMin) & (x < xLowUp)): # LEFT FAST
                            motionProxy.setWalkTargetVelocity(0, 0, 0.3, 0.2)
                            print "leftFast"

                        elif((x > xHighUp) & (x <= xMax)): # RIGHT FAST
                            motionProxy.setWalkTargetVelocity(0, 0, -0.3, 0.2)
                            print "rightFast"

                        elif((x >= xLowUp) & (x <= xHighUp)): # LOOK DOWN
                            headPitch = 0.5
                ...
                ###### LOOKING DOWN CONDITIONALS
                    ...
                    #### If the ball is at the top of the screen
                    if((y >= yMin) & (y < yLowDown)):

                        if((x >= xLowDown) & (x <= xHighDown)): # FORWARD FAST
                            motionProxy.setWalkTargetVelocity(1, 0, 0, 0.5)
                            print "Forward Fast"

                        elif((x >= xMin) & (x < xLowDown)): # LEFT SLOW
                            motionProxy.setWalkTargetVelocity(0, 0, 0.2, 0.2)
                            print "leftSlow"

                        elif((x > xHighDown) & (x <= xMax)): # RIGHT SLOW
                            motionProxy.setWalkTargetVelocity(0, 0, -0.2, 0.2)
                            print "rightSlow"

                    #### If the ball is at the middle of the screen
                    elif((y >= yLowDown) & (y <= yHighDown)):
                        if((x >= xLowDown) & (x <= xHighDown)): # FORWARD SLOW
                            motionProxy.setWalkTargetVelocity(0.8, 0, 0, 0.2)
                            print "Forward slow"

                        elif((x >= xMin) & (x < xLowDown)): # LEFT MEDIUM
                            motionProxy.setWalkTargetVelocity(0, 0, 0.2, 0.2)
                            print "leftMedium"

                        elif((x > xHighDown) & (x <= xMax)): # RIGHT MEDIUM
                            motionProxy.setWalkTargetVelocity(0, 0, -0.2, 0.2)
                            print "rightMedium"

                    #### If the ball is at the bottom of the screen
                    elif((y > yHighDown) & (y <= yMax)):

                        if((x >= xMin) & (x < xLowDown)): # LEFT FAST
                            motionProxy.setWalkTargetVelocity(0, 0, 0.3, 0.2)
                            print "leftFast"

                        elif((x > xHighDown) & (x <= xMax)): # RIGHT FAST
                            motionProxy.setWalkTargetVelocity(0, 0, -0.3, 0.2)
                            print "rightFast"

                        elif((x >= xLowDown) & (x <= xHighDown)): # KICK / LOOK DOWN
                            headPitch = 0
                            stillWorking = False
                            postureProxy.goToPosture("StandInit", 0.8)
                            time2.sleep(1)
